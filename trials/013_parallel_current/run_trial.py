...
def run_trial():
    ...
    for _ in range(config.max_ticks):
        ...
        # move each electron toward the neighbor with the strongest echo
        neighbors_a = engine.get_neighbors(*electron_a.position)
        next_a = max(neighbors_a, key=lambda p: engine.echo_fields[p].rho_local)
        electron_a.position = next_a

        neighbors_b = engine.get_neighbors(*electron_b.position)
        next_b = max(neighbors_b, key=lambda p: engine.echo_fields[p].rho_local)
    ...
    result_path = os.path.join(os.path.dirname(__file__), "parallel_current_results.json")
