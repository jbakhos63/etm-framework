...
def run_trial():
    ...
    for _ in range(config.max_ticks):
        ...
        if engine.tick > 0:
            # move away from regions of high echo (repulsion)
    ...
    result_path = os.path.join(os.path.dirname(__file__), "electron_repulsion_results.json")
