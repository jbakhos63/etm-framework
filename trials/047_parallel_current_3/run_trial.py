...
def run_trial(max_ticks: int = 500, lattice_size=(51, 51, 51)):
    config = ConfigurationFactory.validated_foundation_config("parallel_current")
    ...
    for _ in range(config.max_ticks):
        ...
        neighbors_a = engine.get_neighbors(*electron_a.position)
        electron_a.position = max(neighbors_a, key=lambda p: engine.echo_fields[p].rho_local)

        neighbors_b = engine.get_neighbors(*electron_b.position)
        electron_b.position = max(neighbors_b, key=lambda p: engine.echo_fields[p].rho_local)
    ...
    result_path = os.path.join(os.path.dirname(__file__), "parallel_current_3_results.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extended parallel current trial")
